<?php
/**
 * @phan-file-suppress PhanTypeSuspiciousEcho, PhanTypeMismatchArgumentInternalProbablyReal, PhanTypeSuspiciousStringExpression, PhanUnreferencedUseNormal,PhanUnusedPublicMethodParameter
 */
defined('B_PROLOG_INCLUDED') and (B_PROLOG_INCLUDED === true) or die();
defined('ADMIN_MODULE_NAME') or define('ADMIN_MODULE_NAME', 'iqdev.options');
...
if($RIGHT >= "R") :
    ...
    <form method="post" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=urlencode($module_id)?>&amp;lang=<?=LANGUAGE_ID?>">
        ...
        foreach ($aTabs as $aTab) {
            ...
            if ($aTab['DIV'] != 'access') {
                foreach ($arAllOptions[$aTab['DIV']] as $arOption):
                    //COption::SetOptionString($module_id, $aTab['DIV'].'.'.$arOption[0], $arOption[3]);
                    ...
                    <tr>
                        ...
                        <td width="60%">
                            ...
                            <? elseif ($type[0] == 'select'): ?>
                                ...
                                echo $aTab['DIV'].'['.htmlspecialcharsbx($arOption[0]).']' ?>" id="<?
                                echo htmlspecialcharsbx($arOption[0]) ?>">
                                    <? foreach ($type[1] as $key => $sItem): ?>
                                    <option value="<?= $key?>" <?= $val == $key ? 'selected' : ''?>><?= $sItem?></option>
                                    <? endforeach;?>
                                </select>
                            <? endif ?>
                        </td>
